"""Web search retrieval via Brave Search, Exa, Serper, and ddgr (DuckDuckGo CLI)."""
...
# ---------------------------------------------------------------------------
# ddgr (DuckDuckGo CLI — no API key required)
# ---------------------------------------------------------------------------

def ddgr_search(
    query: str, date_range: tuple[str, str], count: int = 5,
) -> tuple[list[dict], dict]:
    """Search DuckDuckGo via the local ddgr CLI binary. Requires ddgr in PATH."""
    ddgr_bin = shutil.which("ddgr")
    if not ddgr_bin:
        return [], {}
    try:
        result = subprocess.run(
            [ddgr_bin, "--json", "-n", str(count), "--", query],
    ...
    items = []
    for i, r in enumerate(hits[:count]):
        if not isinstance(r, dict):
            continue
        url = r.get("url", "")
        if not url:
        ...
        items.append({
            ...
            "date": None,   # ddgr 不返回发布日期
            "relevance": 0.7,
            "why_relevant": "DuckDuckGo web search (ddgr)",
        })
    artifact = {"label": "ddgr", "webSearchQueries": [query], "resultCount": len(items)}
    return items, artifact
...
def web_search(
    ...
    if backend == "ddgr":
        return ddgr_search(query, date_range)
    if backend != "none":
