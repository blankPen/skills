...
def render_compact(report: schema.Report, cluster_limit: int = 8, fun_level: str = "medium") -> str:
    ...
    lines = [
        ...
        *_assistant_safety_lines(),
        f"- 日期范围：{report.range_from} 至 {report.range_to}",
        f"- 来源：{len(non_empty)} 个活跃（{', '.join(_source_label(s) for s in non_empty)}）" if non_empty else "- 来源：无",
        "",
    ]
    ...
    for index, cluster in enumerate(report.clusters[:cluster_limit], start=1):
        lines.append(
            ...
            f"（评分 {cluster.score:.0f}，{len(cluster.candidate_ids)} 条，来源：{', '.join(_source_label(source) for source in cluster.sources)}）"
        )
...
def render_full(report: schema.Report) -> str:
    ...
    for index, cluster in enumerate(report.clusters, start=1):
        lines.append(
            f"### {index}. {cluster.title} "
            f"（评分 {cluster.score:.0f}，{len(cluster.candidate_ids)} 条，来源：{', '.join(_source_label(s) for s in cluster.sources)}）"
        )
        if cluster.uncertainty:
            lines.append(f"- 不确定性：{cluster.uncertainty}")
    ...
    for source in source_order:
        ...
        for item in items:
            ...
            lines.append(f"**{item.item_id}** （评分:{score:.0f}） {item.author or ''} （{item.published_at or '日期未知'}） [{_format_item_engagement(item)}]")
            ...
            if top_comments and isinstance(top_comments[0], dict):
                ...
                for tc in top_comments[:3]:
                    ...
                    lines.append(f"  热门评论（{tc_score} {vote_label}）：{excerpt}")
            ...
            # 字幕区块：YouTube / Bilibili
            if item.source in ("youtube", "bilibili"):
                transcript_file = item.metadata.get("transcript_file", "")
                if transcript_file:
                    lines.append(f"  **完整字幕文件：** `sources/{transcript_file}`")
                highlights = item.metadata.get("transcript_highlights", [])
                if highlights:
                    lines.append(f"  **字幕摘要（前{len(highlights[:15])}条）：**")
                    for hl in highlights[:15]:
                        lines.append(f'    - "{hl[:200]}"')
                elif not transcript_file:
                    # 无字幕文件也无摘要时，内嵌原始片段作兜底
                    transcript = item.metadata.get("transcript_snippet", "")
                    if transcript and len(transcript) > 100:
                        lines.append(f"  <details><summary>完整字幕 ({len(transcript.split())} 词)</summary>")
            ...
            if outcome_prices and item.source == "polymarket":
                ...
                if odds_parts:
                    lines.append(f"  赔率：{' | '.join(odds_parts)}")
                remaining = item.metadata.get("outcomes_remaining") or 0
                if remaining:
                    lines.append(f"  （另有 {remaining} 个选项）")
...
def render_context(report: schema.Report, cluster_limit: int = 6) -> str:
    ...
    freshness_warning = _assess_data_freshness(report)
    if freshness_warning:
        lines.append(f"新鲜度警告：{freshness_warning}")
    lines.append("热门聚类：")
    for cluster in report.clusters[:cluster_limit]:
        ...
        for candidate_id in cluster.representative_ids[:2]:
            ...
            if candidate.snippet:
                lines.append(f"    摘录：{_truncate(candidate.snippet, 180)}")
    if report.warnings:
        lines.append("警告：")
        lines.extend(f"- {warning}" for warning in report.warnings)
    return "\n".join(lines).strip() + "\n"
...
def _render_candidate(candidate: schema.Candidate, prefix: str) -> list[str]:
    ...
    if candidate.snippet:
        lines.append(f"   - 摘录：{_truncate(candidate.snippet, 360)}")
    for tc in _top_comments_list(primary):
        ...
        lines.append(f"   - 热门评论（{score} {vote_label}）：{_truncate(excerpt.strip(), 240)}")
    insight = _comment_insight(primary)
    if insight:
        lines.append(f"   - 洞察：{_truncate(insight, 220)}")
    highlights = _transcript_highlights(primary)
    if highlights:
        lines.append(f"   - 📝 字幕摘要（{len(highlights)}条）：")
        for hl in highlights:
            lines.append(f'     • "{_truncate(hl, 200)}"')
        transcript_file = primary.metadata.get("transcript_file") if primary else None
        if transcript_file:
            lines.append(f"   - 📄 完整字幕：sources/{transcript_file}")
    return lines
...
def _render_stats(report: schema.Report) -> list[str]:
    ...
    if not non_empty_sources:
        lines.append("- 无可用来源数据。")
        lines.append("")
        return lines

    lines.append(
        f"- 证据总计：{total_items} 条，来自 {len(non_empty_sources)} 个来源"
    )
    top_voices = _top_voices_overall(non_empty_sources)
    if top_voices:
        lines.append(f"- 主要声音：{', '.join(top_voices)}")
...
def _transcript_highlights(item: schema.SourceItem | None) -> list[str]:
    if not item or item.source not in ("youtube", "bilibili"):
        return []
    return (item.metadata.get("transcript_highlights") or [])[:15]
