...
def search_and_transcribe(
    ...
    # Step 2: Fetch transcripts for all videos concurrently.
    # Try more candidates than needed because some videos (music, short clips) lack captions.
    attempt_count = len(items)
    candidate_ids = [item["video_id"] for item in items[:attempt_count]]
    _log(f"Fetching transcripts for {attempt_count} videos: {candidate_ids}")
...
def search_youtube_sc(
    ...
    if items:
        attempt_count = len(items)
        _log(f"Fetching SC transcripts for {attempt_count} videos")
