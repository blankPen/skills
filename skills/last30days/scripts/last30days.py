...
def slugify(value: str) -> str:
    # Allow Unicode alphanumeric (including Chinese, Japanese, Korean, etc.)
    slug = re.sub(r"[^\w]+", "-", value.lower()).strip("-")
    slug = slug.replace("_", "-")  # underscores → hyphens
...
def save_output(report: schema.Report, emit: str, save_dir: str, suffix: str = "") -> Path:
    ...
    topic_dir = Path(save_dir).expanduser().resolve() / slug
    topic_dir.mkdir(parents=True, exist_ok=True)
    extension = "json" if emit == "json" else "md"
    out_name = "raw.json" if extension == "json" else "raw.md"
    out_path = topic_dir / out_name
    if out_path.exists():
        out_path = topic_dir / f"raw-{datetime.now().strftime('%Y-%m-%d')}.{extension}"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--source-prefs", help="JSON string of user source preferences (auto-selection overrides)")
    return parser
...
def main() -> int:
    ...
    # 解析用户渠道偏好（来自 SKILL.md 读取偏好文件后传入）
    if args.source_prefs:
        try:
            config["_source_prefs"] = json.loads(args.source_prefs)
        except json.JSONDecodeError as exc:
            sys.stderr.write(f"[SourcePrefs] 无法解析 --source-prefs JSON: {exc}\n")
    ...
    try:
        ...
        if args.save_dir:
            ...
            transcript_save_dir = str(
                Path(args.save_dir).expanduser().resolve() / slug / "sources"
            )
